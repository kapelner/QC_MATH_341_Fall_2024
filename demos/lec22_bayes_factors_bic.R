pacman::p_load(fitdistrplus, wiqid, nycflights13, nimble, VaRES, data.table, ggplot2)
plogistic = function(q, mu, sigma){VaRES::plogistic(q, mu, sigma)}
pt3 = function(q, mean, sd, df){wiqid::pt3(q, mean, sd, df)}
...
#bayes factors using Laplace's priors
set.seed(1)
Nsamp = 1000
central_tendency_param_draws = runif(Nsamp, -10, 10)
scale_params_draws = runif(Nsamp, 0, 10)
df_param_draws = sample(1 : 50, Nsamp, replace = TRUE)
f_norm = mean(dnorm(x, central_tendency_param_draws, scale_params_draws))
f_cauchy = mean(dcauchy(x, central_tendency_param_draws, scale_params_draws))
f_logistic = mean(dlogistic(x, central_tendency_param_draws, scale_params_draws))
f_laplace = mean(ddexp(x, central_tendency_param_draws, scale_params_draws))
f_t = mean(dt(x, df_param_draws, central_tendency_param_draws))
f_norm
f_cauchy
f_logistic
f_t
f_max = max(f_norm, f_cauchy, f_logistic, f_laplace, f_t)
f_max / f_norm
f_max / f_cauchy
f_max / f_logistic
f_max / f_laplace
f_max / f_t
#none of these get a special mention! Bayesian Model Selection is very conservative
