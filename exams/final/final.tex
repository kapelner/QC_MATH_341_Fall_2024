...
\begin{enumerate}[(1)]
    ...
    \item the values of $\sigsq_1, \sigsq_2$ are known
    \item $H_0: \theta_1 - \theta_2 = 0$ or $H_0: \theta_1 = \theta_2$
    \item $\displaystyle\frac{\xbar_1 - \xbar_2}{\sqrt{\frac{\sigsq_1}{n_1} + \frac{\sigsq_2}{n_2}}}$
\end{enumerate}
}}{
\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid$\\ ~\\
...
\subquestionwithpoints{4} Provide a setting where you would use an \textbf{exact two sample T} test. Make sure to indicate (1) the DGP of the samples from the two populations, (2) what is known about the parameter(s) if anything, (3) the null hypothesis and (4) the test statistic. You do not need to state the statistic's null distribution.


\iftoggle{solutions}{\inred{
\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid \normnot{\theta_1}{\sigsq_1}$,\quad $X_{2,1}, \ldots, X_{2,n_2} \iid \normnot{\theta_2}{\sigsq_2}$
    \item $\sigsq_1 = \sigsq_2$ but its value is unknown
    \item $H_0: \theta_1 - \theta_2 = 0$ or $H_0: \theta_1 = \theta_2$
    \item $\displaystyle\frac{\xbar_1 - \xbar_2}{\sqrt{\displaystyle\frac{(n_1-1)s^2_1 + (n_2-1) s^2_2}{n_1 + n_2 -2}\parens{\frac{1}{n_1} + \frac{1}{n_2}}}}$
\end{enumerate}
}}{
\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid$\\ ~\\
...
\subquestionwithpoints{4} Provide a setting where you would use an \textbf{approximate two sample Z} test. Make sure to indicate (1) the DGP of the samples from the two populations, (2) what is known about the parameter(s) if anything, (3) the null hypothesis and (4) the test statistic. You do not need to state the statistic's null distribution.

\iftoggle{solutions}{\inred{

There are many acceptable answers to this question. Below are two:

\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid \bernoulli{\theta_1}$,\quad $X_{2,1}, \ldots, X_{2,n_2} \iid \bernoulli{\theta_2}$
    \item N/A
    \item $H_0: \theta_1 - \theta_2 = 0$ or $H_0: \theta_1 = \theta_2$
    \item $\displaystyle\frac{\xbar_1 - \xbar_2}{\sqrt{\frac{\xbar_1(1-\xbar_1)}{n_1} + \frac{\xbar_2(1-\xbar_2)}{n_2}}}$
\end{enumerate}


\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid f_1(\theta_1,\sigsq_1)$,\quad $X_{2,1}, \ldots, X_{2,n_2} \iid f_2(\theta_2,\sigsq_2)$ \\ where $f_1$ and $f_2$ are either unknown or otherwise non-normal PDF's or PMF's
    \item finite means and variances for both population DGP's
    \item $H_0: \theta_1 - \theta_2 = 0$ or $H_0: \theta_1 = \theta_2$
...
\subquestionwithpoints{4} Provide a setting where you would use an \textbf{approximate two sample T} test. Make sure to indicate (1) the DGP of the samples from the two populations, (2) what is known about the parameter(s) if anything, (3) the null hypothesis and (4) the test statistic. You do not need to state the statistic's null distribution.
...
\subquestionwithpoints{4} Provide a setting where you would use an \textbf{exact two sample F} test. Make sure to indicate (1) the DGP of the samples from the two populations, (2) what is known about the parameter(s) if anything, (3) the null hypothesis and (4) the test statistic. You do not need to state the statistic's null distribution.

\iftoggle{solutions}{\inred{
\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid \normnot{\theta_1}{\sigsq_1}$,\quad $X_{2,1}, \ldots, X_{2,n_2} \iid \normnot{\theta_2}{\sigsq_2}$
    \item nothing is known
    \item $H_0: \sigsq_1 / \sigsq_2 = 1$ or $H_0: \sigsq_1 = \sigsq_2$
    \item $\displaystyle\frac{s^2_1 / n_1}{s^2_2 / n_2}$
\end{enumerate}
}}{
\begin{enumerate}[(1)]
    \item $X_{1,1}, \ldots, X_{1,n_1} \iid$\\ ~\\
...
\problem Consider the following data with sample size $n=50$:

\begin{verbatim}
2.22  2.44  4.11  2.58  2.79  3.98  2.40  2.99  3.82  3.84
3.07  6.00  2.48  5.07  2.39  3.96  2.68  2.82  4.17  4.53
1.78  4.40  3.99  1.83  6.78 26.53  6.49  2.61  3.13  3.09
3.67  8.63  3.22  3.18  4.15  2.56  3.27  2.66  3.61  3.03
4.61  0.04  3.91  1.05  3.42  2.16  2.68  1.13  6.75 10.15    
\end{verbatim}

\noindent Useful summary statistics are as follows: $\xbar = 4.06$ and $s^2 = 13.75$. We do not know the model that generated this data so we wish to select one using the AIC metric. On the top of the following page is a table of models, their number of parameters denoted $K$ and AIC metrics below. They are sorted in order from lowest AIC to highest AIC (but one is missing).
...
\subquestionwithpoints{2} Which is the best fitting iid rv model according to the AIC metric? 
...
Frechet  & 2 &    291.4
    \end{tabular}
\end{table}
\subquestionwithpoints{8} Compute the iid normal model's AIC to one decimal point.
...
&=& n\natlog{2\pi \doublehat{\sigsq}^{MLE}} + \frac{(n-1)s^2}{\frac{n-1}{n} s^2} + 2k \\
&=& 50 \natlog{6.283 \cdot 13.475} + 50 + 2 \cdot 2 \\
&=& 50 (\natlog{6.283 \cdot 13.475} + 1) + 4 \\
&=& 275.8
\eeqn
...
\problem Every time your professor leaves his apartment, he plays a game. When the door swings shut, he holds his key in middair and tries to estimate where the keyhole is. As the door shuts, the key either goes in the keyhole, or it doesn't. Let $\theta$ denote the probability of succeeding in this worthwhile endeavor.

\begin{enumerate}[(a)] 

\subquestionwithpoints{3} Out of the past 17 tries, he hasn't managed to succeed. Using Laplace's prior of indifference, provide the three Bayesian point estimates for $\theta$ as exact as possible. 


\iftoggle{solutions}{\inred{
Under the uniform prior $f(\theta\,|\,\x) = \betanot{\sum x_i + 1}{n - \sum x_i + 1} = \betanot{1}{18}$. Thus,

\beqn
\thetahatmmse = \oneover{19}, \quad \thetahatmmae = \texttt{qbeta}(0.5, 1, 18),\quad \thetahatmap = 0,
\eeqn
}}{~\spc{3}}

\subquestionwithpoints{2} Which of the point estimates above is unrealistic and why?

\iftoggle{solutions}{\inred{
$\thetahatmap = 0$ because it's unrealistic to estimate that this event is impossible.
}}{~\spc{2}}


He collects more data. Of the past 87 tries, he now manages 7 successes. Use this dataset and Laplace's prior of indifference for the remaining questions in this problem.

\subquestionwithpoints{6} Assume Laplace's prior of indifference, compute $\bias{\thetahatmmse}$.

\iftoggle{solutions}{\inred{
\beqn
\bias{\thetahatmmse} = \expe{\frac{\sum X_i + 1}{n + 2}} - \theta = \frac{\expe{\sum X_i} + 1}{n+2} - \frac{(n+2)\theta}{n+2} =  \frac{n\theta + 1 - n\theta - 2\theta}{n+2} = \frac{1 - 2\theta}{n+2}
\eeqn
}}{~\spc{4}}

\subquestionwithpoints{4} Provide a 95\% CI for $\theta$ to three decimal places.

\iftoggle{solutions}{\inred{
\beqn
CI_{\theta, 95\%} = \bracks{\xbar + \pm 1.96 \cdot \sqrt{\overn{\xbar (1- \xbar)}}} = \bracks{\frac{7}{87} + \pm 1.96 \cdot \sqrt{\frac{\frac{7}{87} \cdot \frac{80}{87}}{87}}} = \bracks{0.023, 0.138} 
\eeqn
}}{~\spc{4}}

\subquestionwithpoints{2} Is the CI in the previous problem within the parameter space $\Theta$? Circle one... ~~\iftoggle{solutions}{\inred{yes}}{yes} \quad / no


\subquestionwithpoints{4} Find an expression for the ninety-eight percent CR for $\theta$. 

\iftoggle{solutions}{\inred{
Under the uniform prior $f(\theta\,|\,\x) = \betanot{\sum x_i + 1}{n - \sum x_i + 1} = \betanot{8}{81}$. Thus,

\beqn
CR_{\theta, 98\%} = \bracks{\texttt{qbeta}(0.01, 8, 81), ~\texttt{qbeta}(0.99, 8, 81)}
\eeqn
}}{~\spc{2}} 


\subquestionwithpoints{4} Find an expression for the Bayesian p-value for the test of $H_a: \theta > 10\%$.

\iftoggle{solutions}{\inred{
\beqn
p_{val} = \cprob{H_0}{\x} = \cprob{\theta \leq 0.1}{\x} = \texttt{pbeta}(0.1, 8, 81)
\eeqn
}}{~\spc{1.5}}


\subquestionwithpoints{6} For the next 100 attempts, provide a 95\% interval of the number of future successes.

\iftoggle{solutions}{\inred{
\beqn
p(X_*\,|\,\x) &=& \text{BetaBinomial}\parens{n_*, \sum x_i + 1, n - \sum x_i + 1} \\
&=& \text{BetaBinomial}\parens{100, 8, 81} \\
PI_{X_*, 95\%} &=& \bracks{\texttt{qbetabinom}(0.025, 100, 8, 81), ~ \texttt{qbetabinom}(0.975, 100, 8, 81)}
\eeqn
}}{~\spc{3.7}}
...
\problem We have the following IQ scores measured on a small sample of $n=5$ from a special subpopulation of interest:

\begin{verbatim}
    90.6 102.8  87.5 123.9 104.9
\end{verbatim}

\noindent Assume the mean IQ score by definition is 100 but we don't know the variance in this specific population, and that is our parameter of interest. Its MLE is calculated to be 165.77.

Also, we have a lot of previous data on subpopulations we can use to create an informative prior to make our inference better. Using this previous data, we fit a prior to be $\sigsq \sim \text{Invgamma}(37, 8325)$.

\begin{enumerate}[(a)] 

\subquestionwithpoints{2} Inference on $\sigsq$ can only be considered generalizable and externally valid if this sample of $n=5$ was collected how?

\iftoggle{solutions}{\inred{
Via a simple random sample (SRS)
}}{~\spc{1}} \pagebreak

\subquestionwithpoints{5} The prior given in the problem header is ... circle as many as apply ...\\~\\ objective \quad / \quad \iftoggle{solutions}{\inred{subjective}}{subjective} \quad / \quad \iftoggle{solutions}{\inred{informative}}{informative} \quad / \quad uninformative \quad / \quad indifferent 

\subquestionwithpoints{4} The prior given in the problem header is equivalent to seeing pseudosamples with what standard deviation (i.e., the square root of variance)?

\iftoggle{solutions}{\inred{
\beqn
\alpha &=& 37 = \frac{n_0}{2} \mathimplies n_0 = 74 \\
\beta &=& 8325 = \frac{n_0 \sigsq_0}{2} = 37 \sigsq_0 \mathimplies \sigma = \sqrt{8325 / 37} = 15
\eeqn
}}{~\spc{3}}

% \subquestionwithpoints{4} Find an expression for the Bayesian p-value for the test of $H_a: \sigsq < 225$.
...
\subquestionwithpoints{4} Find the probability the next IQ in this subpopulation is greater than the mean IQ \emph{exactly}.


\iftoggle{solutions}{\inred{
\beqn
f(X_*\,|\,\x,\theta) &=& T_{n_0 + n}\parens{\theta, \frac{n_0\sigsq_0 + n \doublehat{\sigsq}^{MLE}}{n_0 + n}} = T_{74 + 5}\parens{100, \frac{8325 + 5 \cdot 165.77}{74 + 5}} \\
&=& T_{79}\parens{100, 115.9} \\
\cprob{X_* > 100}{\x,\theta} &=& 0.5
\eeqn
}}{~\spc{3}}

\subquestionwithpoints{6} Find an expression for the probability the next IQ in this subpopulation is greater than 130.


\iftoggle{solutions}{\inred{
From the previous question, we found that $f(X_*\,|\,\x,\theta) = T_{79}\parens{100, 115.9}$ thus

\beqn
\cprob{X_* > 130}{\x,\theta} &=& 1 - \texttt{pt.scaled}(130, 79, 100, \sqrt{115.9})
\eeqn
}}{~\spc{5}}\pagebreak
...
\beqn
\Xoneton \iid \text{Log}\normnot{\theta}{\sigsq} := \oneover{\sqrt{2\pi \sigsq x^2}} e^{-\oneover{2\sigsq}(\natlog{x} - \theta)^2} \indic{x > 0} ~~\text{with}~\sigsq~\text{known}
\eeqn 
...
\subquestionwithpoints{9} Find Jeffrey's prior for $\theta$ if proper. If it is not proper, find what it is proportional to.
...
\beqn
\theta \sim \text{ParetoI}(k,\lambda) := \displaystyle\frac{\lambda k^\lambda}{\theta^{\lambda + 1}}\indic{\theta > k}.
\eeqn

\begin{enumerate}[(a)]

\subquestionwithpoints{9} Show that the posterior for $\theta$ is ParetoI and find its parameters.

\iftoggle{solutions}{\inred{
\beqn
f(\theta\,|\,\x) \propto f(\x\,|\,\theta) k(\theta) &\propto& \parens{\prod_{i=1}^n \oneover{\theta}\indic{x_i \in (0, \theta)}} \frac{1}{\theta^{\lambda + 1}}\indic{\theta > k} \\
&\propto& \frac{1}{\theta^n} \parens{\prod_{i=1}^n \indic{x_i  > 0} \indic{x_i < \theta}} \frac{1}{\theta^{\lambda + 1}} \indic{\theta > k} \\
&\propto& \frac{1}{\theta^n} \parens{\prod_{i=1}^n \indic{x_i < \theta}} \frac{1}{\theta^{\lambda + 1}} \indic{\theta > k} \\
&\propto& \frac{1}{\theta^{n + \lambda + 1}} \indic{\theta > x_1} \indic{\theta > x_2} \cdot \ldots \cdot \indic{\theta > x_n} \indic{\theta > k} \\
%&\propto& \frac{1}{\theta^{n + \lambda + 1}} \indic{ \max{\xoneton} < \theta} \indic{\theta > k} \\
&\propto& \frac{1}{\theta^{n + \lambda + 1}} \indic{\theta > \max{\xoneton, k}} \\
&\propto& \text{ParetoI}(\max{\xoneton, k},n + \lambda)
\eeqn
}}{~\spc{8}}
...
\subquestionwithpoints{6} Show that the maximum likelihood estimate of $\theta$ to the nearest three decimal values is 2.645.
